...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($stmt->execute()) {
            echo "<script>alert('Signup successful! Redirecting to timetable...'); window.location.href='stu1.html?name={$name}&rollnumber={$rollnumber}&department={$department}';</script>";
        } else {
